...
export const isImage =
  ({ optional }: IsImageOptions = { optional: false }): CustomValidator =>
  (_value: any, { req, path }: IsImageParameters) => {
    if (req.files[path])
      return checkMimetypeInRequestFile(path, req.files, [
        'image/png',
        'image/jpeg',
      ]);
    return optional;
...
export const isVideo =
  ...
  (_value: any, { req, path }: IsImageParameters) => {
    if (req.files[path])
      return checkMimetypeInRequestFile(path, req.files, ['video/mp4']);
    return optional;
  };
